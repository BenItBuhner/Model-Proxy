  ...
  while (steps < args.maxSteps && performance.now() < deadline) {
    ...
    for (let attempt = 0; attempt < 4 && reply === undefined; attempt++) {
      try {
        ...
        const transient = /No routes were available|All routes failed|empty|HTTP 5\d\d|status[:=]?\s*5\d\d|524|ECONNRESET|ECONNREFUSED|Unable to connect|connection refused|socket hang up|socket connection was closed|fetch failed|operation timed out/i.test(error);
