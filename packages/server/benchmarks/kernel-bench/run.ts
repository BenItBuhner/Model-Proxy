...
export function isTransientBenchError(error: string | undefined): boolean {
  ...
  return /No routes were available|All routes failed|empty stream|empty content|empty body|HTTP 5\d\d|status[:=]\s*5\d\d|524|ECONNRESET|ECONNREFUSED|Unable to connect|connection refused|socket hang up|socket connection was closed|fetch failed|operation timed out/i.test(error);
}
