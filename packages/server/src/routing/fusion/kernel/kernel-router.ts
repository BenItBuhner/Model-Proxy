...
export class FusionKernel {
  ...
  private scratchpadEnabled(run: KernelRun): boolean {
    // Bands are configurable: at F3 the mandatory computation displaced plain reasoning
    // and regressed solved Apex problems; at max it recovered unsolved ones.
    return run.kcfg.compute_scratchpad && run.kcfg.compute_scratchpad_bands.includes(run.band) && run.examples === undefined && run.codeTask === undefined && run.domains.some((d) => run.kcfg.compute_scratchpad_domains.includes(d));
  }
