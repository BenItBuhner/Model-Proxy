...
function AccountsBody(): React.ReactElement {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="credentials / subscriptions"
        title="Accounts"
        description="Attach multiple personal or shared subscription accounts. Tokens refresh automatically and rotate through the normal fallback router."
      />
      ...
      <Panel
        ...
        subtitle={`${accounts.length} visible`}
...
function Metric({ label, value }: { label: string; value: string }): React.ReactElement {
  return (
    <div className="bg-ink-800 p-4 shadow-edge">
      <div className="font-mono text-[10px] uppercase tracking-[0.18em] text-bone-300">
        {label}
      </div>
      <div className="mt-2 font-mono text-lg text-bone-900">{value}</div>
    </div>
  );
}
